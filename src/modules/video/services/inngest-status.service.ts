...
export class InngestStatusService {
  ...
  private static async getDatabaseBasedStatus(eventId: string): Promise<UserFriendlyStatus> {
    try {
      ...
      const statusMapping = {
        ...
        'extracting_keyframes': { status: 'processing' as const, step: 'Creating visual highlights', progress: 40 },
        'uploading_assets': { status: 'processing' as const, step: 'Uploading assets', progress: 60 },
        'generating_summary': { status: 'processing' as const, step: 'Generating AI summary', progress: 80 },
  ...
  private static getCompletedStepsFromDbStatus(status: string): string[] {
    const allSteps = ['Extracting transcript', 'Creating visual highlights', 'Uploading assets', 'Generating AI summary'];
    const stepMap: Record<string, string[]> = {
      'pending': [],
      'extracting_transcript': [],
      'extracting_keyframes': [allSteps[0]],
      'uploading_assets': [allSteps[0], allSteps[1]],
      'generating_summary': [allSteps[0], allSteps[1], allSteps[2]],
      'completed': allSteps,
      'failed': []
    };
